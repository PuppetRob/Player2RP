...
RegisterNetEvent("loaf_realtor:transfer_property", function(transferTo, propertyId, isRent, shellId)
    local src = source
    ...
    if isRent then
        exports.loaf_housing:GiveProperty(transferTo, function(received)
            if received then
                Notify(src, Strings["rented_property"])
            end
        end, propertyId, "rent", shellId)
    else
...
RegisterNetEvent("loaf_realtor:add_property", function(data)
    local entrance, garageEntrance, garageExit, interiorType, interior, category, propertyType, name, price, rent, misc = data.entrance, data.garageEntrance, data.garageExit, data.interiorType, data.interior, data.category, data.propertyType, data.name, data.price, data.rent, data.misc
    ...
    MySQL.Async.fetchScalar("SELECT MAX(`id`) FROM `loaf_houses`", {}, function(id)
        ...
        MySQL.Async.execute([[
            ...
            (`id`, `label`, `house_apart`, `interior_type`, `interior`, `category`, `entrance`, `price`, `garage_entrance`, `garage_exit`, `misc`, `rent`)
            VALUES
            (@id, @name, @propertyType, @interiorType, @interior, @category, @entrance, @price, @garageEntrance, @garageExit, @misc, @rent)
        ]], {
            ["@id"] = nextId,
            ...
            ["@misc"] = misc and json.encode(misc),
            ["@rent"] = rent
        }, function(added)
            if added == 1 then
                local garage
                ...
                exports.loaf_housing:AddHouse(nextId, {
                    ...
                    doors = misc and misc.doors,
                    locations = misc and misc.locations
                })
            end
        end)
    end)
...
function SetHouses()
    MySQL.Async.fetchAll("SELECT * FROM `loaf_houses`", {}, function(res)
        ...
        for _, v in pairs(res) do
            ...
            local misc = v.misc and json.decode(v.misc)
            if misc then
                for k, v in pairs(misc.doors) do
                    v.coords = vector4(v.coords.x, v.coords.y, v.coords.z, v.coords.w)
                end
                for k, v in pairs(misc.locations) do
                    v.coords = vector3(v.coords.x, v.coords.y, v.coords.z)
                    v.scale = vector3(v.scale.x, v.scale.y, v.scale.z)
                end
            end
