...
AddEventHandler('PlasmaLobby:CurrentGameDATA', function () -- Detection of start 
	...
	-- TriggerServerEvent("PlasmaLobby:SetMetaData",true) --Modif For Hotbar on inventory --NOT WORKING
end)
...
-- PlasmaLobby:CLILeaveTheGame
	...
	Wait(100)
	reEnableAction()
	-- TriggerServerEvent("PlasmaLobby:SetMetaData",false) --Modif For Hotbar on inventory --NOT WORKING
end)
...
AddEventHandler('PlasmaLobby:CurrentWeapon', function(wep) -- Detection of current Weapon (specific to random weap)
	print("CurrentWeapon : "..tostring(wep))
	currentWeapon = wep
end)
...
Citizen.CreateThread(function()
    while true do
        if isCurrentlyInAGame == true then
			...
			SetCurrentPedWeapon(PlayerPedId(),GetHashKey(currentWeapon),true)
        else
