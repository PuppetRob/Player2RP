...
local function updateOdo(dist, veh, plate, possMods)
	...
	if Config.System.Debug and seat == -1 then
		...
		"<br>Manual: "..VehicleStatus[plate].manual..
		"<br>Underglow: "..VehicleStatus[plate].underglow
	end
...
RegisterNetEvent("jim-mechanic:Client:EnteredVehicle", function()
	...
	if VehicleStatus and VehicleStatus[plate] and VehicleStatus[plate]["manual"] == 1 then
	...
	CreateThread(function()
		...
		local ejectLoop = false
		while GetVehiclePedIsIn(Ped, false) == veh and not IsThisModelABicycle(model) and not IsThisModelATrain(model) do
			if ShowOdo then
				if seat == -1 or Config.Odometer.ShowToAll then updateOdo(dist, veh, plate, possMods) end
			end
			if IsVehicleStopped(veh) then wait = 5000 else wait = 10 end -- delay loop when car is stopped
			SetPedHelmet(Ped, false)

			--Beta Ejection System
			if Config.Harness.HarnessControl and Config.Harness.AltEjection then
				...
				if not HasHarness() then
					...
					SetFlyThroughWindscreenParams(seatBeltOn() and minimumSeatBeltSpeed or minimumSpeed, 1.0, 17.0, 10.0)
				else
				...
				if Config.vehFailure.damages then
					CreateThread(function()
						if damageLoop then return else damageLoop = true end
				...
				--Old Ejection System
				local lastSpeed, lastSpeed2, damagedone, frameBodyChange, tick = 0, 0, false, 0, 0
				CreateThread(function()
					if ejectLoop then return else ejectLoop = true end
					while GetVehiclePedIsIn(Ped, false) == veh do
						...
						if lastSpeed < (Config.Harness.minimumSpeed or 60) then
							Wait(100)
							tick = 0
						end
						...
						if tick > 0 then
							...
							if lastSpeed2 > lastSpeed then
								lastSpeed = thisFrameSpeed
							elseif lastSpeed2 < lastSpeed then
								tick = 25
							end
						end
