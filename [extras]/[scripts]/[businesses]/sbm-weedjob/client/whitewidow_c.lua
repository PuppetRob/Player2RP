...
--[[ 		local finished2 = exports["reload-skillbar"]:taskBar(math.random(7000,10000),math.random(1,2))
      --  local finished2 = exports["reload-skillbar"]:taskBar(math.random(2500,5000),math.random(3,5))
        if finished2 ~= 100 then
			QBCore.Functions.Notify("Failed to harvest cbd white-widow..", "error", 3500)
			ClearPedTasks(playerPed)
        else ]]
--[[ 			local finished3 = exports["reload-skillbar"]:taskBar(math.random(7000,10000),math.random(1,2))
          --  local finished3 = exports["reload-skillbar"]:taskBar(math.random(900,2000),math.random(5,7))
            if finished3 ~= 100 then
				QBCore.Functions.Notify("Failed to harvest cbd white-widow..", "error", 3500)
				ClearPedTasks(playerPed)
            else ]]
				local HarvestTime = math.random(1100,1200)
...
RegisterNetEvent('sbm-weedjob:client:UseSkunkJoint', function()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
				isSmokingJoint = true
			--	exports['ProgressBar']:drawBar(5000, 'Smoking some cbd skunk..') 
...
RegisterNetEvent('sbm-weedjob:client:UseOGKushJoint', function()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
				isSmokingJoint = true
			--	exports['ProgressBar']:drawBar(5000, 'Smoking some cbd og kush..') 
				TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_SMOKING_POT", 0, false)
				Wait(5500)
				ClearPedTasks(playerPed)
				Wait(1000)
...
RegisterNetEvent('sbm-weedjob:client:UseWhiteWidowJoint', function()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
				isSmokingJoint = true
			--	exports['ProgressBar']:drawBar(5000, 'Smoking some cbd white widow..') 
				TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_SMOKING_POT", 0, false)
				Wait(5500)
				ClearPedTasks(playerPed)
				Wait(1000)
...
RegisterNetEvent('sbm-weedjob:client:UseAK47Joint', function()
	...
	QBCore.Functions.TriggerCallback('QBCore:HasItem', function(HasItem)
		if HasItem then
				isSmokingJoint = true
			--	exports['ProgressBar']:drawBar(5000, 'Smoking some cbd ak47..') 
				TaskStartScenarioInPlace(playerPed, "WORLD_HUMAN_SMOKING_POT", 0, false)
				Wait(5500)
				ClearPedTasks(playerPed)
				Wait(1000)
