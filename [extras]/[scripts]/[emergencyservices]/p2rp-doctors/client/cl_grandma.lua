...
RegisterNetEvent("ren-grandma:request:help", function()
    if QBCore.Functions.GetPlayerData().metadata.isdead or QBCore.Functions.GetPlayerData().metadata.inlaststand then 
        if QBCore.Functions.GetPlayerData().money['cash'] >= Shared.GrandmaCost then 
            QBCore.Functions.Progressbar('name_here', 'The Doctor is healing you..', 5000, false, false, {
            ...
            QBCore.Functions.Notify("The Doc won't help, if you don't have some cash! Get the fuck out of here.", "error", 5000)
        end
    else
        QBCore.Functions.Notify("You look healthy dumbass", "error")
    end
end)
        ...
        options = {
            {
                ...
                label = 'Request help from the Doctor - '..Shared.GrandmaCost..'$',
            },
        },
        distance = 2
    })
