CreateThread(function()
    ...
    function PurchaseMenu(propertyId)
        ...
        if (Config.Realtor.enabled and not Config.Realtor.requireRealtor) or not Config.Realtor.enabled then
            ...
            if houseData.price then
                table.insert(elements, {
                    header = Strings["property_price"]:format(houseData.price),
                    isMenuHeader = true
                })
            end
            if houseData.rent then
                table.insert(elements, {
                    header = Strings["property_rent"]:format(houseData.rent),
                    isMenuHeader = true
                })
            end
        end
...
CreateThread(function()
    ...
    RegisterNetEvent("qb-houses:client:enterOwnedHouse", function(location)
        TriggerEvent("loaf_housing:spawn_in_property", location)
    end)
end)