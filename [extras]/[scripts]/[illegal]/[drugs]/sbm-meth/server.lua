...
RegisterServerEvent('sbm-meth:start')
AddEventHandler('sbm-meth:start', function()
	...
	if ItemAcetone ~= nil and ItemLithium ~= nil and ItemMethlab ~= nil then
		if ItemAcetone.amount >= 5 and ItemLithium.amount >= 2 and ItemMethlab.amount >= 1 then	
			TriggerClientEvent("sbm-meth:startprod", _source)
...
RegisterServerEvent('sbm-meth:stopf')
AddEventHandler('sbm-meth:stopf', function(id)
local _source = source
	local xPlayers = QBCore.Functions.GetPlayers()
	local xPlayer = QBCore.Functions.GetPlayer(tonumber(source))
	for i=1, #xPlayers, 1 do
		TriggerClientEvent('sbm-meth:stopfreeze', xPlayers[i], id)
	end	
end)

RegisterServerEvent('sbm-meth:make')
AddEventHandler('sbm-meth:make', function(posx,posy,posz)
	local _source = source
	local xPlayer = QBCore.Functions.GetPlayer(tonumber(source))
	if xPlayer.Functions.GetItemByName('methlab') ~= nil then
		if xPlayer.Functions.GetItemByName('methlab').amount >= 1 then	
			local xPlayers = QBCore.Functions.GetPlayers()
			for i=1, #xPlayers, 1 do
				TriggerClientEvent('sbm-meth:smoke',xPlayers[i],posx,posy,posz, 'a') 
			end		
		else
			TriggerClientEvent('sbm-meth:stop', _source)
		end
	else
...
RegisterServerEvent('sbm-meth:finish')
AddEventHandler('sbm-meth:finish', function(qualtiy)
	local _source = source
	local xPlayer = QBCore.Functions.GetPlayer(tonumber(source))
...
RegisterServerEvent('sbm-meth:blow')
AddEventHandler('sbm-meth:blow', function(posx, posy, posz)
	local _source = source
	local xPlayers = QBCore.Functions.GetPlayers()
	local xPlayer = QBCore.Functions.GetPlayer(tonumber(source))
	for i=1, #xPlayers, 1 do
		TriggerClientEvent('sbm-meth:blowup', xPlayers[i],posx, posy, posz)
	end
