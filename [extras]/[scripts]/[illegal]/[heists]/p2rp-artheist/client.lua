                    ...
                    if not v['taken'] then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            if weapon == GetHashKey('WEAPON_SWITCHBLADE') then
                                ...
                                QBCore.Functions.Notify('You dont armed a switchblade')
                            end
                        end
                    end
                end
        ...
        if status then
            QBCore.Functions.TriggerCallback('artheist:server:checkRobTime', function(time)
                if time then
                    ...
                    stealBlip = addBlip(vector3(-424.6, 25.45, 46.23), 355.39, 0, Strings['steal_blip'])
                    repeat
                        local ped = PlayerPedId()
                        local pedCo = GetEntityCoords(ped)
...
function HeistAnimation(sceneId)
    ...
    if ArtHeist['cut'] == 1 then
      ...
      --  TriggerServerEvent("p2rp-artheist:server:callCops", "Art Gallery", streetLabel, coords)
...
AddEventHandler('artheist:client:showNotification', function(str)
