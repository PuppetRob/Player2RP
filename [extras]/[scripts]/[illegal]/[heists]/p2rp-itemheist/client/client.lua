...
function StartMission()
    if CurrentCops >= Config.RequiredPolice then
        if StartCoolDownRemaining <= 0 then
            StartCoolDown()
            TriggerServerEvent("p2rp-itemheist:server:SetActive", true)
            TriggerServerEvent("p2rp-itemheist:SendDiscordLog")
            SendEmail()
        else
...
RegisterNetEvent("p2rp-itemheist:SpawnTruck")
AddEventHandler("p2rp-itemheist:SpawnTruck", function()
    ...
    QBCore.Functions.SpawnVehicle("barracks", function(veh)
        ...
        exports['p2rp-fuel']:SetFuel(veh, 100.0)
...
function SendEmail()
    TriggerServerEvent(Config.Phone..':server:sendNewMail', {
        sender = "Paige",
        subject = "Item Mission",
        message = "So You Need Some Rare Items? <br /> <strong/> I Set The Location For You On The GPS </strong>",
        button = {}
    }) 
    MissionBlip()
end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if Distance < 60.0 then
                ...
                TriggerServerEvent("p2rp-itemheist:Server:GuardsSync", -1)
                TriggerEvent("p2rp-itemheist:SpawnTruck")
...
RegisterNetEvent("p2rp-itemheist:ResetMission")
AddEventHandler("p2rp-itemheist:ResetMission", function()
    StartPeds = {}
    TriggerServerEvent("p2rp-itemheist:server:SetActive", false)
    InBuyerPoint = false
    InMilitaryPoint = false
end)

RegisterNetEvent('p2rp-itemheist:client:SetActive', function(status)
    isActive = status
end)
...
RegisterNetEvent('p2rp-itemheist:GuardsSync', function()
    SpawnGuards()
end)
