...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('p2rp-jewellery:server:GetJewelleryState', function(result)
...
AddEventHandler('QBCore:Client:OnPlayerUnload', function()
  for i = 1, #Config.Vitrines do
    if Config.Vitrines[i].isBusy then
      TriggerServerEvent('p2rp-jewellery:server:SetVitrineState', false, i)
    end
  end
  removeBlips()
end)
...
AddEventHandler('onResourceStart', function(resource)
  ...
  TriggerServerEvent('p2rp-jewellery:server:StoreHit', 'all', false)
  createBlips()
end)
...
AddEventHandler('p2rp-jewellery:client:SmashCase', function(case)
  QBCore.Functions.TriggerCallback('p2rp-jewellery:server:GetCops', function(cops)
        ...
        if cops >= Config.RequiredCops then
          if isStoreHit(case, false) or isStoreHacked() then
            ...
            QBCore.Functions.Progressbar('smash_vitrine', Lang:t('info.smashing_progress'), Config.WhitelistedWeapons[pedWeapon].timeOut, false, true, {
              ...
              TriggerServerEvent('p2rp-jewellery:server:VitrineReward', case)
              TriggerServerEvent('p2rp-jewellery:server:SetTimeout', case)
              ...
              TriggerServerEvent('p2rp-jewellery:server:SetVitrineState', 'isBusy', false, case)
              smashing = false
              TaskPlayAnim(ped, animDict, 'exit', 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('p2rp-jewellery:server:SetVitrineState', 'isBusy', true, case)
        ...
        if dist <= 1.5 then
          if QBCore.Functions.HasItem(Config.DoorItem) then
            ...
            exports['ps-ui']:Thermite(function(success) -- success
              if success then
                TriggerServerEvent('p2rp-jewellery:server:StoreHit', store, true)    
                ...
                Wait(4000)
                TriggerServerEvent('p2rp-jewellery:server:RemoveDoorItem')
                ...
                TriggerEvent('p2rp-jewellery:client:HackSuccess', store)
...
AddEventHandler('p2rp-jewellery:client:HackSecurity', function()
  QBCore.Functions.TriggerCallback('p2rp-jewellery:server:GetCops', function(cops)
    if not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
      if Config.Skills.enabled then
          ...
          if QBCore.Functions.HasItem(Config.HackItem) then
            ...
            exports['ps-ui']:VarHack(function(success)
              if success then
                ...
                hacking = false
                TriggerServerEvent('p2rp-jewellery:server:StoreHit', 'all', true)
                Wait(250)
                StopAnimTask(ped, animDict, anim, 8.0)
                DeleteEntity(tab)
                TriggerEvent('p2rp-jewellery:client:HackSuccess')
              else
                hacking = false
...
AddEventHandler('p2rp-jewellery:client:HackSuccess', function(store)
    ...
    if isStoreHit(store, true)  and not isStoreHacked() then
      if not Config.OneStore then
        ...
        if Config.AutoLock then
          TriggerServerEvent('p2rp-jewellery:server:ToggleDoorlocks', store, false, false)
          locked = false
        ...
        if Config.AutoLock then TriggerServerEvent('p2rp-jewellery:server:ToggleDoorlocks', store, false, false) end
        locked = false
      ...
      if Config.AutoLock and not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
        TriggerServerEvent('p2rp-jewellery:server:ToggleDoorlocks', store, true, false)
        locked = true
      end
      TriggerServerEvent('p2rp-jewellery:server:StoreHit', store, false)
    else
      if not Config.OneStore then 
        QBCore.Functions.Notify(Lang:t('success.hacked_threestore'), 'success')
        if Config.AutoLock then TriggerServerEvent('p2rp-jewellery:server:ToggleDoorlocks', nil, false, true) end
        locked = false
        Wait(Config.Cooldown)
        if Config.AutoLock and not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
          locked = true
          TriggerServerEvent('p2rp-jewellery:server:ToggleDoorlocks', nil, true, true)
        end
        TriggerServerEvent('p2rp-jewellery:server:StoreHit', 'all', false)
      else
...
RegisterNetEvent('p2rp-jewellery:client:SetVitrineState', function(stateType, state, k)
...
RegisterNetEvent('p2rp-jewellery:client:StoreHit', function(storeIndex, isHit)
      ...
      options = {
        {
          ...
          action = function()
            if validWeapon() then
              TriggerEvent('p2rp-jewellery:client:SmashCase', k)
            else
      ...
      options = {
        {
          ...
          item = 'thermite',
          action = function()
            TriggerEvent('p2rp-jewellery:client:Thermite', k)
          end
        }
      },
      ...
      options = {
        {
          ...
          action = function()
            if GetSelectedPedWeapon(ped) == `WEAPON_UNARMED` then
              ...
              if validWeapon() then
                TriggerEvent('p2rp-jewellery:client:SmashCase', i)
              else
                QBCore.Functions.Notify(Lang:t('error.wrong_weapon'), 'error')
              end
            end
    ...
    options = {
      {
        ...
        action = function()
          TriggerEvent('p2rp-jewellery:client:Thermite', 1)
        end
      }
    },
    distance = 2.5 -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
...
}, 
{
  options = {
    ...
    action = function()
        TriggerEvent('p2rp-jewellery:client:HackSecurity')
      end
    }
  },
  distance = 2.5 -- This is the distance for you to be at for the target to turn blue, this is in GTA units and has to be a float value
    ...
    if LocalPlayer.state.isLoggedIn then
      if not checkTime(Config.VangelicoHours.range.open, Config.VangelicoHours.range.close) then
        ...
        if not loopDone then
          Wait(1000)
          TriggerServerEvent('p2rp-jewellery:server:ToggleDoorlocks', nil, false, true)
          loopDone = true
        end
      end
    end
