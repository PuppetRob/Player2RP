...
--[[ local function exploitBan(id, reason)
  ...
  string.format('%s was banned by %s for %s', GetPlayerName(id), 'p2rp-jewellery', reason), true)
  DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end ]]

-------------------------------- EVENTS --------------------------------

RegisterServerEvent('p2rp-jewellery:server:RemoveDoorItem', function()
  local src = source
...
RegisterServerEvent('p2rp-jewellery:server:SetVitrineState', function(stateType, state, k)
  if stateType == 'isBusy' and type(state) == 'boolean' and Config.Vitrines[k] then
    Config.Vitrines[k][stateType] = state
    TriggerClientEvent('p2rp-jewellery:client:SetVitrineState', -1, stateType, state, k)
  end
end)

RegisterServerEvent('p2rp-jewellery:server:StoreHit', function(storeIndex, bool)
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  if not Player then return end
  TriggerClientEvent('p2rp-jewellery:client:StoreHit', -1, storeIndex, bool)
...
RegisterServerEvent('p2rp-jewellery:server:ToggleDoorlocks', function(store, locked, allStores)
  local src = source
...
RegisterServerEvent('p2rp-jewellery:server:VitrineReward', function(vitrineIndex)
  local src = source
  local Player = QBCore.Functions.GetPlayer(src)
  local cheating = false
  if not Config.Vitrines[vitrineIndex] or Config.Vitrines[vitrineIndex].isOpened then 
    exploitBan(src, 'Trying to trigger an exploitable event \"p2rp-jewellery:server:VitrineReward\"') 
    return 
  end
  ...
  if CachedPoliceAmount[src] >= Config.RequiredCops then
    if plrPed then
      ...
      if dist <= 25.0 then
        ...
        TriggerClientEvent('p2rp-jewellery:client:SetVitrineState', -1, 'isOpened', true, vitrineIndex)
        TriggerClientEvent('p2rp-jewellery:client:SetVitrineState', -1, 'isBusy', false, vitrineIndex)
  ...
  if cheating then
    ...
    if Flags[license] >= 3 then
      exploitBan('Getting flagged many times from exploiting the \"p2rp-jewellery:server:VitrineReward\" event')
    else
...
RegisterServerEvent('p2rp-jewellery:server:SetTimeout', function(vitrine)
  local store = 0
  ...
  if not TimeOuts[store] then
    TimeOuts[store] = true
    TriggerEvent('qb-scoreboard:server:SetActivityBusy', 'vangelicojewelry', true)
    CreateThread(function()
      ...
      TriggerClientEvent('p2rp-jewellery:client:StoreHit', -1, 'all', false)
      for i = 1, #Config.Vitrines do
        Config.Vitrines[i].isOpened = false
        TriggerClientEvent('p2rp-jewellery:client:SetVitrineState', -1, 'isOpened', false, i)
        TriggerClientEvent('p2rp-jewellery:client:SetAlertState', -1, false)
        TriggerEvent('qb-scoreboard:server:SetActivityBusy', 'vangelicojewelry', false)
      end
...
QBCore.Functions.CreateCallback('p2rp-jewellery:server:GetCops', function(source, cb)
  local src = source
	local amount = 0
  for _, v in pairs(QBCore.Functions.GetQBPlayers()) do
    if v.PlayerData.job.name == "lspd" and v.PlayerData.job.onduty or v.PlayerData.job.name == "bcso" and v.PlayerData.job.onduty then
      amount = amount + 1
    end
  end
...
QBCore.Functions.CreateCallback('p2rp-jewellery:server:GetJewelleryState', function(_, cb)
