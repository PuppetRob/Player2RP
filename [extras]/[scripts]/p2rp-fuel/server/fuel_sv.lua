...
if Config.RenewedPhonePayment then
	RegisterNetEvent('p2rp-fuel:server:phone:givebackmoney', function(amount)
		local src = source
...
RegisterNetEvent("p2rp-fuel:server:OpenMenu", function(amount, inGasStation, hasWeapon, purchasetype, FuelPrice)
	local src = source
	...
	if inGasStation == true and not hasWeapon then
		if Config.RenewedPhonePayment and purchasetype == "bank" then
			TriggerClientEvent("p2rp-fuel:client:phone:PayForFuel", src, amount)
		else
			if Config.Ox.Menu then
				if Config.FuelDebug then print("going to open the context menu (OX)") end
				TriggerClientEvent('p2rp-fuel:client:OpenContextMenu', src, total, amount, purchasetype)
			else
...
RegisterNetEvent("p2rp-fuel:server:PayForFuel", function(amount, purchasetype, FuelPrice, electric)
	local src = source
	if not src then return end
	local Player = QBCore.Functions.GetPlayer(src)
	if not Player then return end
...
RegisterNetEvent("p2rp-fuel:server:purchase:jerrycan", function(purchasetype)
...
if Config.UseJerryCan then
	QBCore.Functions.CreateUseableItem("jerrycan", function(source, item)
		local src = source
		TriggerClientEvent('p2rp-fuel:jerrycan:refuelmenu', src, item)
	end)
end
...
RegisterNetEvent('p2rp-fuel:info', function(type, amount, srcPlayerData, itemdata)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
CreateThread(function()
	updatePath = "/CodineDev/p2rp-fuel"
	resourceName = "p2rp-fuel ("..GetCurrentResourceName()..")"
