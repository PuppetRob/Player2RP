...
QBCore.Functions.CreateUseableItem('scratch_ticket', function(source)
  ...
  TriggerClientEvent("p2rp-scratch:isActiveCooldown", source)
end)

RegisterNetEvent("p2rp-scratch:handler", function(returncooldown, cooldown)
  ...
  if count >= 1 then
    ...
    TriggerClientEvent("p2rp-scratch:setCooldown", _source)
  ...
  TriggerClientEvent("p2rp-scratch:startScratchingEmote", _source)
  ...
  for key,priceInfo in pairs(Config.Prices) do
    ...
    if randomNumber > add and randomNumber <= add + chance then
      ...
      TriggerClientEvent("p2rp-scratch:nuiOpenCard", _source, key, price, amount, price_type, price_label)
      return price
    end
    add = add + chance
  end
...
RegisterNetEvent("p2rp-scratch:deposit", function(key, price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
...
RegisterNetEvent("p2rp-scratch:stopScratching", function(price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
  local tempsrc = tonumber(_source)
