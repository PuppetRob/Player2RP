			...
			while true do
				...
				if give then
					...
					if IsControlJustPressed(0, Config.ConfirmGiveButton) then
						...
						TriggerServerEvent('devcore_smokev2:server:Receiver', playerId, item, size, prop, type, time)
	...
	if type == 'cigarette' or type == 'cigar' or  type == 'joint' then
		...
		if IsEntityInWater(playerPed) then
			...
			if not give and IsControlJustPressed(0, Config.SmokeButton) then
				...
				SetEntityVisible(mouthsmokeprop, false, 0)
...
function GiveArmor(type)
	if type == 'cigarette' and Config.AddArmor == 'cigarette' then
		AddArmourToPed(GetPlayerPed(-1), Config.Armor)
	end
	if type == 'cigar' and Config.AddArmor == 'cigar' then
		AddArmourToPed(GetPlayerPed(-1), Config.Armor)
	end
	if type == 'joint' and Config.AddArmor == 'joint' then
		AddArmourToPed(GetPlayerPed(-1), Config.Armor)
	end
	if type == 'vape' and Config.AddArmor == 'vape' then
		AddArmourToPed(GetPlayerPed(-1), Config.Armor)
	end
	if type == 'bong' and Config.AddArmor == 'bong' then
		AddArmourToPed(GetPlayerPed(-1), Config.Armor)
	end
	if Config.AddArmor == 'all' then
		AddArmourToPed(GetPlayerPed(-1), Config.Armor)
	end
	if Config.AddArmor == 'off' then

	end
	if type == 'cigarette' and Config.StressSystem == 'cigarette' then
		TriggerEvent('devcore_smokev2:client:triggerstressqb')
	end
	if type == 'cigar' and Config.StressSystem == 'cigar' then
		TriggerEvent('devcore_smokev2:client:triggerstressqb')
	end
	if type == 'joint' and Config.StressSystem == 'joint' then
		TriggerEvent('devcore_smokev2:client:triggerstressqb')
	end
	if type == 'vape' and Config.StressSystem == 'vape' then
		TriggerEvent('devcore_smokev2:client:triggerstressqb')
	end
	if type == 'bong' and Config.StressSystem == 'bong' then
		TriggerEvent('devcore_smokev2:client:triggerstressqb')
	end
	if Config.StressSystem == 'all' then
		TriggerEvent('devcore_smokev2:client:triggerstressqb')
	end
	if Config.StressSystem == 'off' then

end
end

RegisterNetEvent('devcore_smokev2:client:triggerstressqb')
AddEventHandler('devcore_smokev2:client:triggerstressqb', function()
	StressTrigger()
end)
