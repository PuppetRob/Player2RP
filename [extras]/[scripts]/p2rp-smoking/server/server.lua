...
AddEventHandler('devcore_smokev2:server:Receiver', function(playerId, item, size, prop, type, time)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local ReceiverPlayer = QBCore.Functions.GetPlayer(playerId)
    if ReceiverPlayer ~= nil then
    TriggerClientEvent("devcore_smokev2:client:Receiver", ReceiverPlayer.PlayerData.source, item, size, prop, type, time)
    end
end)
