        ...
        --if isLoggedIn then
            for k, v in pairs(Config.RentalPoints) do
                ...
                if dist < 10 then
                    ...
                    if dist < 1 then
                        DrawText3Ds(Config.RentalPoints[k]["coords"][1]["x"], Config.RentalPoints[k]["coords"][1]["y"], Config.RentalPoints[k]["coords"][1]["z"] + 0.35, '~g~E~w~ - Rent Vehicle')
...
-- Citizen.CreateThread(function()
    -- for k, v in pairs(Config.RentalPoints) do
        ...
        -- SetBlipScale  (RentalPoints, 0.7)
        -- SetBlipAsShortRange(RentalPoints, true)
        -- SetBlipColour(RentalPoints, 3)

        -- BeginTextCommandSetBlipName("STRING")
        -- AddTextComponentSubstringPlayerName("Rental - Pickup Point")
    ...
    -- for k, v in pairs(Config.DeliveryPoints) do
        ...
        -- SetBlipScale  (DeliveryPoints, 0.7)
...
RentVehicleMenu = function()
    ...
    for k, v in pairs(Config.RentalVehicles) do
        Menu.addButton(Config.RentalVehicles[k]["label"].." - Cost: $"..Config.RentalVehicles[k]["price"], "RentVehicle", k)
    end
...
RentVehicle = function(selectedVehicle)
    ...
    if IsPedInAnyVehicle(ped) then
        QBCore.Functions.Notify('You can only rent vehicles while on foot', 'error')
        return
    end

    local vehiclePlate = "RENT"..math.random(11, 90)
    TriggerServerEvent('kartrental:server:SetVehicleRented', vehiclePlate, true, Config.RentalVehicles[selectedVehicle])

    RentedVehicleData = selectedVehicle
end

RegisterNetEvent('kartrental:server:SpawnRentedVehicle')
AddEventHandler('kartrental:server:SpawnRentedVehicle', function(vehiclePlate, vehicleData)
    local ped = GetPlayerPed(-1)
    local coords = {
        x = Config.RentalPoints[CurrentRentalPoint]["coords"][1]["x"],
        y = Config.RentalPoints[CurrentRentalPoint]["coords"][1]["y"],
        z = Config.RentalPoints[CurrentRentalPoint]["coords"][1]["z"],
    }
   ...
   -- exports['qb-vehiclekeys']:GiveKeys(GetVehicleNumberPlateText(veh), true)
...
ReturnVehicle = function()
    ...
    QBCore.Functions.Notify('You have no returns to be made', 'error')
end
...
AcceptReturn = function()
    ...
    if noSpace(VehiclePlate) ~= noSpace(RentedVehiclePlate) then
        ...
        TriggerServerEvent('kartrental:server:SetVehicleRented', RentedVehiclePlate, false, Config.RentalVehicles[RentedVehicleData])
