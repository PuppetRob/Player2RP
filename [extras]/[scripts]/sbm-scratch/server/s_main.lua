...
QBCore.Functions.CreateUseableItem('scratch_ticket', function(source)
  ...
  TriggerClientEvent("sbm-scratch:isActiveCooldown", source)
end)

RegisterNetEvent("sbm-scratch:handler", function(returncooldown, cooldown)
  ...
  if count >= 1 then
    ...
    TriggerClientEvent("sbm-scratch:setCooldown", _source)
  ...
  TriggerClientEvent("sbm-scratch:startScratchingEmote", _source)
  ...
  for key,priceInfo in pairs(Config.Prices) do
    ...
    if randomNumber > add and randomNumber <= add + chance then
      ...
      TriggerClientEvent("sbm-scratch:nuiOpenCard", _source, key, price, amount, price_type, price_label)
      return price
    end
    add = add + chance
  end
...
RegisterNetEvent("sbm-scratch:deposit", function(key, price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
...
RegisterNetEvent("sbm-scratch:stopScratching", function(price, amount, type)
  local _source = source
  local playerName, playerIdentifier = GetPlayerName(_source), GetPlayerIdentifier(_source, 0)
  local tempsrc = tonumber(_source)
