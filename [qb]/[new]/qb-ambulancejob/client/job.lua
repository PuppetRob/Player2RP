...
function TakeOutVehicle(vehicleInfo)
    ...
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
        ...
        exports['sbm-fuel']:SetFuel(veh, 100.0)
...
RegisterNetEvent('QBCore:Client:OnJobUpdate', function(JobInfo)
    PlayerJob = JobInfo
     if PlayerJob.name == 'ambulance' or PlayerJob.name == 'firefighter' or PlayerJob.name == 'firefighter' then
    ...
    CreateThread(function()
        ...
        QBCore.Functions.GetPlayerData(function(PlayerData)
            ...
            elseif (PlayerData.metadata["inlaststand"] and not PlayerData.metadata["isdead"]) then
             ...
             if PlayerJob.name == 'ambulance' or PlayerJob.name == 'firefighter' or PlayerJob.name == 'firefighter' and onDuty then
                TriggerServerEvent("hospital:server:AddDoctor", PlayerJob.name)
            end
        end)
    end)
...
RegisterNetEvent('QBCore:Client:OnPlayerUnload', function()
     if PlayerJob.name == 'ambulance' or PlayerJob.name == 'firefighter' and onDuty then
        TriggerServerEvent("hospital:server:RemoveDoctor", PlayerJob.name)
    end
end)

RegisterNetEvent('QBCore:Client:SetDuty', function(duty)
     if PlayerJob.name == 'ambulance' or PlayerJob.name == 'firefighter' or PlayerJob.name == 'firefighter' and duty ~= onDuty then
        if duty then
            TriggerServerEvent("hospital:server:AddDoctor", PlayerJob.name)
        else
            TriggerServerEvent("hospital:server:RemoveDoctor", PlayerJob.name)
